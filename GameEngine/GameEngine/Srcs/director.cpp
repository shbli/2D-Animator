...
void Director::setupDirector() {
    ...
    if (GraphicView == 0)
    {
        ...
        //save window size
        desk.setWidth(QApplication::desktop()->screenGeometry(0).width());
        desk.setHeight(QApplication::desktop()->screenGeometry(0).height());
...
//                qWarning() << "GL is invalid";
            ...
            qDebug() << "Game Engine :: OpenGL " << openGLwidget->format().majorVersion() << "." << openGLwidget->format().minorVersion();
...
bool Director::position_in_sprite(QPointF position, Sprite *sprite_to_check) {
