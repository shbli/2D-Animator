...
Sprite* Sprite::spriteWithImage(const QString filename) {
    Sprite* newsprite = new Sprite(QString(filename));
    newsprite->SpriteFileName = QString(filename);
    return newsprite;
}

Sprite *Sprite::spriteWithSvg(const QString filename, float scaleValue)
{
