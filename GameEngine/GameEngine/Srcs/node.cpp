...
void Node::updateMyOpacity(qreal parentOpacity) {
...
void Node::updateMyRotation(qreal paretRotate) {
...
void Node::setEffectivePos()
{
    ...
    emit updateChildsPos(realTimePoint.x(),realTimePoint.y());
}
...
void Node::updateMyPos(qreal parentX,qreal parentY) {
    parent_real_x = parentX;
    parent_real_y = parentY;
    setEffectivePos();
}

void Node::updateMyScale(qreal parentxScale, qreal parentyScale) {
...
void Node::schedualUpdate() {
        connect(Director::sharedDirector(),SIGNAL(update(double)),this,SLOT(update(double)),Qt::UniqueConnection);
}
...
QPointF Node::bottomleftPoint() {
    return QPointF(center_x - (XscaleFactor*w*x_anchor),center_y - (YscaleFactor*h*y_anchor));
}
